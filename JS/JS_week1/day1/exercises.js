...
let elements = ['kerouac', 'fante', 'fante', 'buk', 'hemingway', 'hornby', 'kerouac', 'buk', 'fante'];
...
function fibs(n) {
  ...
  let lastIndex = prevFibs.length - 1;
...
function zeroSum(nums) {
  ...
  for (let i = 0; i < nums.length - 1; i++) {
    let firstIterationNum = nums[i];
    for (let j = i + 1; j < nums.length; j++) {
      let secondIterationNum = nums[j];
